export function calculateAffordability(income, expenses, savings, itemPrice, itemName) {
    ...
    let score, label, verdict;
    if (gap <= 0) {
        score = 95;
        label = 'Ready now';
        verdict = 'You can afford this today from your savings.';
    }
    else if (monthsNeeded <= 3) {
        score = 82;
        label = 'Almost there';
        verdict = 'A short save sprint and this is yours.';
    }
    else if (monthsNeeded <= 9) {
        score = 65;
        label = 'Doable';
        verdict = 'Achievable with some discipline on spending.';
    }
    else if (monthsNeeded <= 24) {
        score = 42;
        label = 'Stretch goal';
        verdict = "You'll need a solid plan — but it's realistic.";
    }
    else {
        score = 20;
        label = 'Not yet';
        verdict = 'This needs a bigger income or longer timeline.';
    }
    const impacts = [];
    if (pctOfSavings > 80) {
        impacts.push({
...
export function formatCurrency(amount, currency = 'THB') {
    try {
        ...
        const map = { THB: '฿', USD: '$', EUR: '€', JPY: '¥' };
        const sym = map[currency] ?? currency + ' ';
        return sym + Math.round(amount).toLocaleString();
    }
}
export function monthsToReadable(months) {
    if (months <= 0)
        return 'Now';
    if (months < 12)
