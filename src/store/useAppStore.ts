...
import type { User, Goal, Transaction, Vault, RetirementPlan } from '@/types'
...
interface AppState {
  ...
  vaults: Vault[]
  retirementPlan: RetirementPlan | null
  userDataById: Record<string, UserScopedState>
  ...
  addVault: (vault: Omit<Vault, 'id' | 'createdAt'>) => void
...
const persistActiveUserState = (state: Pick<AppState, 'activeUserId' | 'goals' | 'transactions' | 'vaults' | 'retirementPlan' | 'userDataById' | 'currency'>) => {
  if (!state.activeUserId) return state.userDataById
  ...
  persist(
    (set, get) => ({
      ...
      depositToVault: (id, amount) => set((state) => {
        ...
        const nextVaults = state.vaults.map((vault) => (
          vault.id === id ? { ...vault, currentAmount: vault.currentAmount + amount } : vault
        ))
